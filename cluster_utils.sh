...
#    CMD="ps aux | grep -v 'grep' | grep 'python code_template' | awk -F' ' '{print $2}' | xargs kill -9"
    CMD="ps aux | grep -v 'grep' | grep -v 'bash' | grep -v 'ssh' | grep 'python lstm_cluster' | awk -F' ' '{print \$2}' | xargs kill -9"
    for i in `seq 0 2`; do
        ssh node$i "$CMD"
    done
}
...
function start_cluster() {
    if [ -z $2 ]; then
        ...
        if [ "$2" = "single" ]; then
            ...
            nohup ssh node3 "cd ~/tf ; python3 $1 --deploy_mode=cluster2  --task_index=3" > serverlog-3.out 2>&1&
            nohup ssh node4 "cd ~/tf ; python3 $1 --deploy_mode=cluster2  --task_index=4" > serverlog-4.out 2>&1&
        fi
    fi
}
